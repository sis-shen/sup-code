...
func (a *Agent) RunPlan(ctx context.Context, sessionID string, input string) (*pkg.AgentResult, error) {
    ...
    if a.sessionManager != nil {
        ...
        a.sessionManager.SetState(ctx, sessionID, pkg.StateWaitingApproval)
    }
...
func (a *Agent) ApprovePlan(ctx context.Context, sessionID string) (*pkg.AgentResult, error) {
    ...
    systemPrompt, _, err := a.contextManager.BuildContext(ctx, sessionID)
    if err != nil {
        a.setState(StateError)
        return &pkg.AgentResult{Error: fmt.Sprintf("build context: %v", err)}, fmt.Errorf("build context: %w", err)
    }
    ...
    if a.sessionManager != nil {
        a.sessionManager.SetState(ctx, sessionID, pkg.StateActing)
    }
    ...
    for stepIndex, step := range plan.Steps {
        ...
        case <-ctx.Done():
            ...
            return &pkg.AgentResult{
                Plan:    plan,
                Summary: fmt.Sprintf("execution cancelled after step %d/%d", stepIndex, len(plan.Steps)),
                Error:   ctx.Err().Error(),
            }, ctx.Err()
        default:
        }
        ...
        if err != nil {
            if isRetryableErr(err) {
                result, retryErr := a.retryStep(ctx, sessionID, step, toolSchemas, stepIndex+1, err)
                if retryErr != nil {
                    a.setState(StateError)
                    return &pkg.AgentResult{
                        Plan:    plan,
