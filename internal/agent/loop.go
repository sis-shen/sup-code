...
func (a *Agent) Run(ctx context.Context, sessionID string, input string) (*pkg.AgentResult, error) {
	...
	if a.sessionManager != nil {
		...
		if err := a.sessionManager.SetState(ctx, sessionID, pkg.StatePlanning); err != nil {
			log.Printf("set state planning: %v", err)
		}
	}
	...
	for stepIndex, step := range plan.Steps {
		...
		case <-ctx.Done():
			...
			return &pkg.AgentResult{
				Plan:    plan,
				Summary: fmt.Sprintf("execution canceled after %d/%d steps", stepIndex+1, len(plan.Steps)),
				Error:   ctx.Err().Error(),
			}, ctx.Err()
		default:
		}
		...
		if err != nil {
			if isRetryableErr(err) {
				corrected, retryErr := a.retryStep(ctx, step, toolSchemas, stepIndex+1, err)
				if retryErr != nil {
					a.setState(StateError)
					return &pkg.AgentResult{
						Plan:    plan,
...
func (a *Agent) executeStep(ctx context.Context, sessionID string, step pkg.PlanItem, tools []pkg.ToolSchema, stepNum int) (string, error) {
...
func (a *Agent) retryStep(ctx context.Context, step pkg.PlanItem, tools []pkg.ToolSchema, stepNum int, originalErr error) (string, error) {
