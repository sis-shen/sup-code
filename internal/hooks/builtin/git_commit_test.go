...
func TestGitCommitHook_InGitRepo(t *testing.T) {
	tmpDir := t.TempDir()
	runCmd(t, tmpDir, "init")
	runCmd(t, tmpDir, "config", "user.email", "test@test.com")
	runCmd(t, tmpDir, "config", "user.name", "Test")

	initialFile := filepath.Join(tmpDir, "initial.txt")
	require.NoError(t, os.WriteFile(initialFile, []byte("initial"), 0644))
	runCmd(t, tmpDir, "add", "initial.txt")
	runCmd(t, tmpDir, "commit", "-m", "initial")
	...
	out := runCmd(t, tmpDir, "log", "--oneline")
...
func runCmd(t *testing.T, dir string, args ...string) []byte {
	t.Helper()
	cmd := exec.Command("git", args...)
	cmd.Dir = dir
	out, err := cmd.CombinedOutput()
	require.NoError(t, err, "cmd git %v failed: %s", args, string(out))
	return out
}
